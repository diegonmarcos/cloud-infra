...
# ║                                                                  ║
# ║   Source : 1_cicd/src/scripts/cloud-ship-repo-workflow-engine.sh
...
do_deploy() {
    ...
    if [ -f "$REPO_ROOT/.gitmodules" ]; then
        ...
        # Keep in sync with 9_others/build.sh — that bootstrap copy must stand
        # alone, so the logic is duplicated there.
