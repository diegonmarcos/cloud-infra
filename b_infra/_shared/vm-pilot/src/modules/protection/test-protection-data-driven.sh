...
command -v nix-instantiate >/dev/null 2>&1 || {
  # #413: on CI a skip is a lie — this tester is registered, and a registered
  # tester that exits 0 without asserting anything reads as coverage. The
  # registered-testers job installs nix; if that ever regresses, fail loudly.
  if [ -n "${GITHUB_ACTIONS:-}" ]; then
    echo "::error::test-protection-data-driven.sh: nix-instantiate missing on CI — the registered-testers job must install nix"
    exit 1
  fi
...
let
  ...
  wdDisk = wd.home.file.".local/share/system-protection/disk-watchdog.sh".text;
  pmText = (import ./infra/prune-maintenance.nix { vmName = "oci-mail"; } { inherit config pkgs lib; })
             .home.file.".local/share/system-protection/infra-prune-maintenance.sh".text;
  # #413: the floor the rendered scripts must carry is READ from the same
  # consolidated snapshot the modules read — never a number typed here.
  cons = builtins.fromJSON (builtins.readFile ./_cloud-data-consolidated.json);
  floor = toString (((cons._home_manager.vms."oci-mail" or {}).protection or {}).journal_retention_floor_days
            or cons.native.protection.journal_retention_floor_days);
