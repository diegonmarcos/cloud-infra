...
# ║                                                                  ║
# ║   Source : 1_configs/src/deploy/scripts/cloud-ship-container-step-build-nix.sh
...
step_build() {
    ...
    CLOUD_DATA_STAGED=""
    CLOUD_DATA_DIR="$SERVICE_DIR/../../1_configs/dist"
    ...
    if [ -n "${CLOUD_DATA_PRESTAGED_BY_CI:-}" ]; then
        ...
        if [ -n "$FLOCK_BIN" ]; then
            (
                ...
                git -C "$SERVICE_DIR/../.." -c submodule."1_configs/dist".update=checkout \
                    submodule update --remote --init --force 1_configs/dist 2>&1 \
                    | while IFS= read -r line; do log "  $line"; done
            ) 9>/tmp/cloud-submodule-update.lock || \
                log "WARN: 1_configs/dist update failed — ship will use the pinned commit (may be stale)"
        else
            git -C "$SERVICE_DIR/../.." -c submodule."1_configs/dist".update=checkout \
                submodule update --remote --init --force 1_configs/dist 2>&1 \
                | while IFS= read -r line; do log "  $line"; done || \
    ...
    # v2 engine flake reads 1_configs/dist/ directly; skip src/ injection
    ...
    if [ "$IS_V2_ENGINE_PRE" = "true" ] && [ "$DEREF_SYMLINKS" != "true" ]; then
        log "v2 engine flake — cloud-data accessed via 1_configs/dist, no src/ resolve needed"
        ...
        # include_cloud_data=true: also copy every 1_configs/dist/*.json into src/ (for services
        # that need the whole dataset at runtime, e.g. c3-infra-mcp-api).
        # NEVER for v2 engine flakes (2026-07-03): they read 1_configs/dist
    ...
    # v2 layout: flake reads 1_configs/dist directly via builtins.readDir;
    ...
    # Include 1_configs/dist/ files in dist/ for runtime use (e.g. C3 API needs topology)
    ...
    if [ "$INCLUDE_CLOUD_DATA" = "true" ] && [ "$IS_V2_ENGINE" = "false" ]; then
        ...
        if [ -d "$CLOUD_DATA_DIR" ]; then
            ...
            log "Included 1_configs/dist/*.json + *.md in dist/"
        fi
