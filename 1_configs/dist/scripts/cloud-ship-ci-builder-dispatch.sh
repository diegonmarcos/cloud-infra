...
# ║                                                                  ║
# ║   Source : 1_configs/src/deploy/scripts/cloud-ship-ci-builder-dispatch.sh
...
# → oci-apps cloud-builder-x). Source of truth: 1_configs/build.json
# (.runners), materialised by 1_configs/build.sh into dist/build-workflows.json.
...
if [ -f "$REPO_ROOT/.gitmodules" ] && [ -d "$REPO_ROOT/1_configs/dist" ]; then
  ...
  (
    ...
    log "Updating 1_configs/dist submodule (once, serialized for parallel ship jobs)"
    if ! git -C "$REPO_ROOT" -c submodule."1_configs/dist".update=checkout \
...
CLOUD_DATA_DIR="$REPO_ROOT/1_configs/dist"
