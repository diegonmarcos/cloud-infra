...
# ║                                                                  ║
# ║   Source : 1_configs/src/gha/scripts/cloud-ship-repo-config-gen.sh
...
#   cloud-data/1_configs/src/gha/scripts/cloud-data-config.ts
#
...
cmd_config() {
    ...
    # The derive job moved out of 1_configs when that module was reduced to
    ...
    if [ -z "$_cd_base" ]; then
        for _p in "$CLOUD_ROOT/1_configs/dist" "$(dirname "$CLOUD_ROOT")/cloud-data" "$_ws/cloud-data"; do
            [ -f "$_p/1_configs/src/gha/scripts/cloud-data-config.ts" ] && { _cd_base="$_p"; break; }
        done
    fi
    CD_SCRIPTS="$_cd_base/1_configs/src/gha/scripts"
