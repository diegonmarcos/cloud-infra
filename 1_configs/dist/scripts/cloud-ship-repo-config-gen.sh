...
# ║                                                                  ║
# ║   Source : 1_configs/src/scripts/cloud-ship-repo-config-gen.sh
...
#   cloud/1_configs/src/engines/        <- the .ts engines (cloud-data-config.ts, derive, etc.)
...
#   cloud-data/1_configs/src/scripts/cloud-data-config.ts
#
...
cmd_config() {
    ...
    if [ -x "$CLOUD_ROOT/1_configs/build.sh" ]; then
        ...
        ENGINES_DIR="$CLOUD_ROOT/1_configs/src/engines"
    ...
    if [ -z "$_cd_base" ]; then
        for _p in "$CLOUD_ROOT/1_configs/dist" "$(dirname "$CLOUD_ROOT")/cloud-data" "$_ws/cloud-data"; do
            [ -f "$_p/1_configs/src/scripts/cloud-data-config.ts" ] && { _cd_base="$_p"; break; }
        done
    fi
    CD_SCRIPTS="$_cd_base/1_configs/src/scripts"
