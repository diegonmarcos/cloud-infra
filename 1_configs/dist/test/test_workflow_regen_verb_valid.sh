...
# ║                                                                  ║
# ║   Source : 1_configs/src/deploy/test/test_workflow_regen_verb_valid.sh
...
# 1_configs/build.sh verb.
#
# Surfaced 2026-04-27 (ship run 25010189292): 4 cicd templates ran
# `bash 1_configs/build.sh config` — but `config` is not a valid verb;
...
#
# This tester reads the actual verb list from 1_configs/build.sh's case
...
for f in "$CICD_DIR"/*.yml; do
    [ -f "$f" ] || continue
    # Lines like:  run: bash 1_configs/build.sh <verb>
    # Only consider lines that actually invoke the script — `run: bash 1_configs/build.sh <verb>`.
    # Excludes `name:` lines that mention the script in human-readable comments.
    while IFS= read -r line; do
        [ -z "$line" ] && continue
        EXAMINED=$((EXAMINED + 1))
        verb=$(printf '%s' "$line" | sed -nE 's|.*bash 1_configs/build\.sh ([a-z][a-z-]*).*|\1|p')
        if [ -z "$verb" ]; then
            continue
        fi
    ...
    done < <(grep -E 'bash 1_configs/build\.sh [a-z]' "$f" || true)
done

if [ "$EXAMINED" -eq 0 ]; then
    echo "  (no 1_configs/build.sh invocations found in any cicd template)"
fi

if [ "$FAIL" -eq 1 ]; then
    echo
    echo "::error::At least one cicd template invokes 1_configs/build.sh with an invalid verb"
...
echo
echo "All cicd 1_configs/build.sh invocations use valid verbs."
