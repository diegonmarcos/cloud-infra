...
#   <p>/1_configs/src/scripts/cloud-data-config.ts
...
# moved to cloud/1_configs/src/engines/ and the canonical entry point
...
#   1. The script tries cloud/1_configs/build.sh FIRST (current layout).
...
SCRIPT="$REPO/1_configs/src/scripts/cloud-ship-repo-config-gen.sh"
FAILS=0

[ -f "$SCRIPT" ] || { echo "✗ $SCRIPT not found"; exit 1; }

# 1. Current layout — 1_configs/build.sh dispatch is present and FIRST.
if grep -q '1_configs/build.sh.*all\|"$CLOUD_ROOT/1_configs/build.sh"' "$SCRIPT"; then
    echo "✓ script dispatches to 1_configs/build.sh (current engine layout)"
else
    echo "✗ script does not dispatch to 1_configs/build.sh — engine reorg not handled"
    FAILS=$((FAILS + 1))
fi

# Path A must come before Path B in the script (current preferred over legacy).
path_a_line=$(grep -n '1_configs/build.sh' "$SCRIPT" | head -1 | cut -d: -f1)
...
if grep -q 'cloud-data-config\.ts' "$SCRIPT"; then
    echo "✓ legacy fallback (1_configs/src/scripts/cloud-data-config.ts) still present"
else
    echo "✗ legacy fallback removed — back-compat broken"
    FAILS=$((FAILS + 1))
fi

# 3. Resolves on the actual checkout: 1_configs/build.sh exists and is executable
if [ -x "$REPO/1_configs/build.sh" ]; then
    echo "✓ 1_configs/build.sh is executable on this checkout"
else
    echo "✗ 1_configs/build.sh missing or non-executable — engine reorg incomplete"
    FAILS=$((FAILS + 1))
fi

# 4. The engine .ts file the legacy fallback expected lives at the new path
if [ -f "$REPO/1_configs/src/engines/cloud-data-config.ts" ]; then
    echo "✓ cloud-data-config.ts at new location: 1_configs/src/engines/"
else
    echo "✗ 1_configs/src/engines/cloud-data-config.ts missing"
    FAILS=$((FAILS + 1))
fi

# 5. Diagnostic on failure: error message lists what was tried.
if grep -q 'SKIP: neither 1_configs/build.sh nor cloud-data legacy engine found' "$SCRIPT"; then
