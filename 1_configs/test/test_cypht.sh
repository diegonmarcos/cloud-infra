...
# Run after `cloud/1_configs/build.sh` regenerates dist/ and after
...
if [ -f "$CADDY_JSON" ]; then
    ...
    fail_with "build-caddy.json not found — run cloud/1_configs/build.sh first"
fi
...
BC_JSON="$CLOUD_ROOT/1_configs/dist/build-cypht.json"
...
check "compose.nix: NO per-container extra_hosts (routing is Caddy's job, hickory wildcards to it)" \
    bash -c "! grep -qE '^[[:space:]]*extra_hosts' '$COMPOSE_NIX'"
