...
# Run AFTER `cloud/1_configs/build.sh` regenerates dist/.
...
# ── Maddy: dual-bind (wg0 + loopback, transitional) ─────────────────────────
# Loopback retained while etc-hosts-clean.nix is in flight — once oci-mail's
# /etc/hosts hijack is stripped, the loopback duplicates can be dropped.
check "maddy: IMAPS 993 dual-binds loopback (transitional)" \
  grep -qE 'tls://127\.0\.0\.1:993' "$MADDY_CONF"
check "maddy: SMTPS 465 dual-binds loopback (transitional)" \
