...
do_build() {
    ...
    # Purge ONLY what this engine owns. dist/ is now shared with the config
    # engine (consolidate/derive write build-*.json and
    # _cloud-data-consolidated.json here since 1_workflows was absorbed), so
    # the old blanket `rm -rf "$DIST_DIR"` would delete every generated config
    # and leave ~700 build-json symlinks across a_solutions/ and b_infra/
    # dangling. Deletions in src/ still propagate, because each owned subtree
    # is removed in full.
    step "purging this engine's artifacts under $(relp "$DIST_DIR")/ (dist/ is shared with the config engine — see comment)"
    rm -rf "$DIST_DIR/scripts" "$DIST_DIR/hooks" "$DIST_DIR/test" "$DIST_DIR/actions"
    rm -f  "$DIST_DIR"/*.yml "$DIST_DIR"/.gitmodules "$DIST_DIR"/.gitignore \
           "$DIST_DIR"/.gitattributes "$DIST_DIR"/gitconfig \
           "$DIST_DIR"/flake.nix "$DIST_DIR"/flake.lock
