...
// Script lives at: cloud/1_configs/src/engines — 2 levels up = cloud/1_configs
...
function deriveGhaConfig(c: any): DerivedFile {
  ...
  // 1_configs/src/scripts/cloud-ship-orchestrate-{ghcr,vm}.sh) prefer this
...
function main() {
  ...
  const PIPELINE_META = {
    ...
    engines: {
      consolidator: "1_configs/src/engines/cloud-data-config-consolidated.ts",
      derive: "1_configs/src/engines/cloud-data-config-derive.ts (THIS engine)",
    },
    outputs: {
    ...
    if (typeof file.data === "object" && !Array.isArray(file.data)) {
      ...
      output = {
        ...
        _meta: {
          ...
          generated_by: "1_configs/src/engines/cloud-data-config-derive.ts",
