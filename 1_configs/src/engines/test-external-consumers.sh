...
#
# 1_configs/src/external-consumers.json is the declarative registry of
# non-container consumers. 1_configs/src/engines/cloud-data-config-derive.ts
# (deriveExternalConsumers + getDeep helpers) emits one
# 1_configs/dist/build-{name}.json per registry entry, sliced from
...
DIST="$REPO/1_configs/dist"
REGISTRY="$REPO/1_configs/src/external-consumers.json"
