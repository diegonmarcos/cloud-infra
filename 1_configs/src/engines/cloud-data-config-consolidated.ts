...
const CLOUD_DATA_DIR = join(CONFIGS_DIR, "dist");             // sole write target (cloud/1_configs/dist)
...
function main() {
  ...
  // Repo NAMES are not sensitive and live in this public repo by design. The
  // TOKEN used to fetch them is — it stays in the vault / GH secrets and is
  // never written here. Refreshed by .github/workflows/fetch-github-repos.yml.
  ...
  if (!existsSync(GITHUB_REPOS_JSON)) {
    ...
    const _n = githubInventory.repos?.length ?? 0;
    const _priv = (githubInventory.repos ?? []).filter((r: any) => r?.visibility === "private").length;
    console.log(`  github-repos.json: ${_n} repos (${_priv} private), fetched ${githubInventory.fetched_at ?? "?"}`);
  }
  ...
  const consolidated = {
    ...
    _meta: {
      ...
      generated_by: "1_configs/src/engines/cloud-data-config-consolidated.ts",
      pipeline: {
        ...
        engines: {
          ...
          derive: "1_configs/src/engines/cloud-data-config-derive.ts",
        },
        outputs: {
          master: "1_configs/dist/_cloud-data-consolidated.json (this file)",
          per_container: "1_configs/dist/build-{container_or_service_name}.json (one per container in topology, plus one for each container-less service)",
  ...
  // a_solutions/<svc>/src/_cloud-data-consolidated.json → 1_configs/dist/, but
