...
// Script lives at: cloud/1_configs/src/engine — 2 levels up = cloud/1_configs
 ...
 *  1_configs/src/external-consumers.json. Same shape as the per-container
  ...
  for (const consumer of registry.consumers ?? []) {
    ...
    out.push({
      ...
      data: {
        ...
        _generated: now(),
        _source: "_cloud-data-consolidated.json + 1_configs/src/external-consumers.json via cloud-data-config-derive.ts/external-consumers",
...
function deriveGhaConfig(c: any): DerivedFile {
  ...
  // 1_configs/src/deploy/scripts/cloud-ship-orchestrate-{ghcr,vm}.sh) prefer this
...
function deriveBuildReports(c: any): DerivedFile {
  ...
  // 1_configs/src/inputs/reports-*.json. Read verbatim and embed.
  const ENGINE_DIR = import.meta.dirname!;
  ...
  return {
    ...
    data: {
      _generated: now(),
      _source: "_cloud-data-consolidated.json + 1_configs/src/inputs/reports-*.json via cloud-data-config-derive.ts/build-reports",
      _replaces: [
...
//
// Reads the engine-owned POLICY source (1_configs/src/inputs/notify-policy.json:
...
// infra-obs_ntfy/src/build-notify.json symlink → 1_configs/dist/build-notify.json.
function deriveBuildNotify(c: any): DerivedFile {
  ...
  return {
    ...
    data: {
      ...
      _generated: now(),
      _source:
        "1_configs/src/inputs/notify-policy.json + _cloud-data-consolidated.json#configs.ntfy.topics " +
  ...
  return {
    ...
    data: {
      _meta: {
        ...
        source_inputs: [
          "1_configs/dist/cloud-fleet-containers-declared.json",
          "front/1_configs/dist/front-fleet-gh-declared.json",
        ],
...
function main() {
  ...
  const PIPELINE_META = {
    ...
    engines: {
      consolidator: "1_configs/src/engine/cloud-data-config-consolidated.ts",
      derive: "1_configs/src/engine/cloud-data-config-derive.ts (THIS engine)",
    },
    outputs: {
      master: "1_configs/dist/_cloud-data-consolidated.json (consolidator output)",
      per_container: "1_configs/dist/build-{container_or_service_name}.json (one per container, plus one per container-less service — derive output)",
      archived: "1_configs/dist/z_archive/cloud-data-{slice}.json (deprecated split files — kept for soft-transition fallbacks; consumers should read consolidated instead)",
    },
    ...
    if (typeof file.data === "object" && !Array.isArray(file.data)) {
      ...
      output = {
        ...
        _meta: {
          ...
          generated_by: "1_configs/src/engine/cloud-data-config-derive.ts",
