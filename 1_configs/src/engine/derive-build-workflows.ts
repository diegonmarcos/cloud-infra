...
//
// Reads 1_configs/build.json (probe/dispatch config, plus name) and
// 1_configs/cloud-builders-runner.json (dedicated source-of-truth for
...
//
//   • 1_configs/src/deploy/scripts/cloud-ship-ci-builder-dispatch.sh
//   • 1_configs/src/deploy/scripts/cloud-ship-container-step-build-docker.sh
//   • 1_configs/src/engine/libs/cloud-paths.sh
//   • 1_configs/src/deploy/gha/cicd/lint-pipeline.yml
//   • 1_configs/src/deploy/test/test_runners_*.sh
//   • 1_configs/src/deploy/test/test_arch_runner_routing.sh
//   • 1_configs/src/deploy/test/test_no_hardcoded_builder_image_literals.sh
//
...
// ../../1_configs/dist/build-workflows.json (FIRE-RULE 6 — no hardcoded
...
const SRC_BUILD_JSON = join(CLOUD_ROOT, "1_configs", "build.json");
const SRC_RUNNERS_JSON = join(CLOUD_ROOT, "1_configs", "cloud-builders-runner.json");
...
function main(): void {
  ...
  const out = {
    _comment:
      "Derived by 1_configs/src/engine/derive-build-workflows.ts from 1_configs/build.json (name/probe/dispatch) and 1_configs/cloud-builders-runner.json (runners, when present — falls back to build.json's .runners otherwise). Do not edit — edit the source.",
    _source: "1_configs/build.json + 1_configs/cloud-builders-runner.json",
