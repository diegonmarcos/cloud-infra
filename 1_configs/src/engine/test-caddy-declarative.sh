...
CLOUD_JSON="$REPO/cloud/1_configs/dist/build-caddy.json"
...
if [ ! -e "$CADDY_JSON" ]; then
  echo "  staging: $CADDY_JSON (symlink → ../../../1_configs/dist/build-caddy.json)"
  ln -sf "../../../1_configs/dist/build-caddy.json" "$CADDY_JSON"
fi
