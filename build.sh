...
cmd_workflow() {
    ...
    WF_SRC="$SCRIPT_DIR/1_workflows/src"
    WF_DIST="$SCRIPT_DIR/1_workflows/dist"
    ...
    GHA_CONFIG="$SCRIPT_DIR/I_cloud-data/cloud-data-gha-config.json"
    {
        log "Generating cloud-data-gha-config.json via tsx..."
        C3_SRC="$SCRIPT_DIR/a_solutions/bc-obs_c3-infra-api/src"
        TOPO_FILE="$SCRIPT_DIR/I_cloud-data/cloud-data-topology.json"
...
"
    ...
    if [ -d "$WF_DIST/actions" ]; then
        ...
        cp -r "$WF_DIST/actions" "$SCRIPT_DIR/.github/actions"
        log "  Actions deployed"
    fi

    # deps flake: dist/deps/ → .github/
    if [ -d "$WF_DIST/deps" ]; then
        cp "$WF_DIST/deps/flake.nix" "$SCRIPT_DIR/.github/flake.nix"
        [ -f "$WF_DIST/deps/flake.lock" ] && cp "$WF_DIST/deps/flake.lock" "$SCRIPT_DIR/.github/flake.lock"
    ...
    if [ -f "$WF_DIST/modules/gitmodules" ]; then
        if [ -f "$SCRIPT_DIR/.gitmodules" ]; then
            if ! diff -q "$WF_DIST/modules/gitmodules" "$SCRIPT_DIR/.gitmodules" >/dev/null 2>&1; then
                log "  WARNING: .gitmodules diverged from source — overwriting"
            fi
        fi
        cp "$WF_DIST/modules/gitmodules" "$SCRIPT_DIR/.gitmodules"
        log "  Submodules config deployed"
    fi

    # gitconfig: dist/gitconfig → .gitconfig
    if [ -f "$WF_DIST/gitconfig" ]; then
        cp "$WF_DIST/gitconfig" "$SCRIPT_DIR/.gitconfig"
...
CONFIG:
    ...
    workflow              Generate GHA workflows from build.json + templates
                          (1_workflows/src/*.tpl → 1_workflows/dist/ → .github/workflows/)
