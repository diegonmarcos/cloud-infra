...
10. Regenerate derived JSONs: `cd /home/diego/git/cloud/1_configs && ./build.sh`.
...
Add `cloud/1_configs/test/test_mail_bind.sh`:
```bash
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/.."
...
```
Wire into the consolidated test runner (`cloud/1_configs/build.sh test` or `1_configs` preflight).
