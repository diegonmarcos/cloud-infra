...
2. Update `~/git/cloud-infra/b_infra/_shared/modules/` to ship fail2ban with `sshd` jail enabled (`findtime=10m, maxretry=3, bantime=1h`).
...
2. `cd ~/git/cloud-infra/b_infra && ./build.sh ship` — redeploy HM to all 4 VMs.
