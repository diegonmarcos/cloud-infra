# Plan: Consolidate All Cloud Engines into `9_others/src/deploy/scripts/`
...
- `workflows/` → `9_others/` (git mv + all refs)
- `cloud-data/` → `I_cloud-data/`, `tools/` → `II_tools/` (submodule renames)
- `static/` → `cicd/` (GHA workflow YAMLs)
- All scripts consolidated into `9_others/src/deploy/scripts/` with `cloud-ship-*`/`cloud-health-*` naming
...
- TS engines moved to cloud-data/9_others/src/deploy/scripts/ and renamed: cloud-data-config-consolidated.ts, cloud-data-config-derive.ts, master cloud-data-config.ts (+ gen-gha-config.ts remaining as symlink)
...
**Goal**: `cloud/build.sh` becomes ~100-line pure dispatcher. All logic moves to `9_others/src/deploy/scripts/`.
...
| `cloud-ship-repo-workflow-gen.sh` | `cmd_workflow()` + `9_others/build.sh` | Build workflows src→dist→.github |
...
#### Step 3.4: Remove `9_others/build.sh` (absorbed)
...
4. Phase 3.4 — remove 9_others/build.sh
