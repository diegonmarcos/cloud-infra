...
6. Generate read-only SSH deploy key for `diegonmarcos/cloud-me_vault`
...
- name: Clone vault
  ...
  run: |
    ...
    GIT_SSH_COMMAND="ssh -i ~/.ssh/vault_deploy -o StrictHostKeyChecking=no" \
      git clone --depth 1 git@github.com:diegonmarcos/cloud-me_vault.git "$GITHUB_WORKSPACE/../vault"
```
