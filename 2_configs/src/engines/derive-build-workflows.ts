...
//
// Reads 1_workflows/build.json (probe/dispatch config, plus name) and
// 1_workflows/cloud-builders-runner.json (dedicated source-of-truth for
...
const SRC_RUNNERS_JSON = join(CLOUD_ROOT, "1_workflows", "cloud-builders-runner.json");
...
function main(): void {
  ...
  const out = {
    _comment:
      "Derived by 2_configs/src/engines/derive-build-workflows.ts from 1_workflows/build.json (name/probe/dispatch) and 1_workflows/cloud-builders-runner.json (runners, when present — falls back to build.json's .runners otherwise). Do not edit — edit the source.",
    _source: "1_workflows/build.json + 1_workflows/cloud-builders-runner.json",
