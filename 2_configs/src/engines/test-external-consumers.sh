...
#
# 2_configs/src/external-consumers.json is the declarative registry of
# non-container consumers. 2_configs/src/engines/cloud-data-config-derive.ts
# (deriveExternalConsumers + getDeep helpers) emits one
# 2_configs/dist/build-{name}.json per registry entry, sliced from
...
#
# This test is registered by 2_configs/build.sh `run_tests` (which iterates
# test-*.sh). No imperative arg-passing — fixture is the live engine output.

set -euo pipefail

REPO="${GIT_BASE:-$HOME/git}/cloud"
DIST="$REPO/2_configs/dist"
REGISTRY="$REPO/2_configs/src/external-consumers.json"
...
if [ ! -f "$CONSOLIDATED" ]; then
    fail "$CONSOLIDATED missing — run bash 2_configs/build.sh all"
