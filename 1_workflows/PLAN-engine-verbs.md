...
**Engine touched:** `1_workflows/src/scripts/cloud-ship-container-engine.sh` (+ new step files)
...
5. **`verbs.json` (`1_workflows/src/libs/verbs.json`):** declares composite sequences; dispatcher reads it instead of hardcoding `ship`/`rollout`/`rollback` order. Atomic verbs stay 1:1 with functions (no verb-DSL — avoid over-engineering).
