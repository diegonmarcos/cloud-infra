...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_gitignore_synced.sh
...
# ║   1_workflows/src/gitignore via the workflow engine.             ║
# ║                                                                  ║
...
[ -f "$DIST" ] && pass "dist artifact exists: 1_workflows/dist/.gitignore" || fail "missing dist (run ./1_workflows/build.sh build)"
[ -f "$ROOT" ] && pass "deployed copy exists: .gitignore"                  || fail "missing deploy (run ./1_workflows/build.sh deploy)"
...
if [ -f "$DIST" ] && [ -f "$ROOT" ]; then
    if cmp -s "$DIST" "$ROOT"; then
        ...
        fail "dist/.gitignore diverges from root — rerun ./1_workflows/build.sh"
    fi
fi
