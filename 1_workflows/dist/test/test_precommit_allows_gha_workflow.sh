...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_precommit_allows_gha_workflow.sh
...
# ║   engine paths (.github/workflows/, 1_workflows/src/cicd/,       ║
# ║   1_workflows/dist/) whose filename mentions secrets BUT whose   ║
# ║   content is a proper GHA workflow (has `on:` and `jobs:`).      ║
# ║                                                                  ║
# ║   Non-workflow files with secret-ish names must still be blocked.║
# ║                                                                  ║
# ║ Usage: bash 1_workflows/src/test/test_precommit_allows_gha_...   ║
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
HOOK="$REPO_ROOT/1_workflows/dist/hooks/pre-commit"
