...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_c3_services_api_registry.sh
...
# ║                                                                  ║
# ║ Asserts every service in 2_configs/dist/build-c3-services-api    ║
...
BUILD_FILE="$REPO_ROOT/2_configs/dist/build-c3-services-api.json"

FAIL=0
pass() { printf "  ✓ %s\n" "$1"; }
fail() { printf "  ✗ %s\n" "$1" >&2; FAIL=1; }

[ -f "$BUILD_FILE" ] || { echo "FATAL: missing $BUILD_FILE — run 2_configs/build.sh all" >&2; exit 1; }
