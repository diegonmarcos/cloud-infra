...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_lint_pipeline_referenced_files_tracked.sh
...
# Test: every `bash 1_workflows/src/test/<file>.sh` invocation in
...
LINT="$REPO_ROOT/1_workflows/src/cicd/lint-pipeline.yml"
...
# Extract every `bash 1_workflows/src/test/<file>.sh` (and dist/test/<file>.sh).
while IFS= read -r path; do
    ...
    printf "  OK  %s\n" "$path"
done < <(grep -oE 'bash 1_workflows/(src|dist)/test/[a-z_-]+\.sh' "$LINT" | awk '{print $2}' | sort -u)
