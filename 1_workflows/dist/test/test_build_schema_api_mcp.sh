...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_build_schema_api_mcp.sh
...
# ║                                                                  ║
# ║ Usage: bash 1_workflows/src/test/test_build_schema_api_mcp.sh    ║
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SCHEMA="$REPO_ROOT/a_solutions/build.schema.json"
VALIDATOR="$REPO_ROOT/2_configs/src/engines/validate-build-schema.ts"
...
HARNESS="$REPO_ROOT/2_configs/src/engines/_validate-block-fixture.ts"
...
run_harness() {  # block, fixture-content -> echoes VALID|INVALID (always exit 0)
  ...
  (cd "$REPO_ROOT/2_configs/src/engines" && npx tsx "$HARNESS" "$SCHEMA" "$fpath" "$block" 2>&1) || true
}
...
if (cd "$REPO_ROOT/2_configs/src/engines" && npx tsx "$VALIDATOR" >/dev/null 2>&1); then
