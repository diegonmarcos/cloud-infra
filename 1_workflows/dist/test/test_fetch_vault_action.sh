...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_fetch_vault_action.sh
...
ACTION="$REPO/1_workflows/src/actions/fetch-vault/action.yml"
CICD_DIR="$REPO/1_workflows/src/cicd"
FAILS=0
...
if [ -f "$DEPLOYED" ]; then
    ...
    if diff -q "$ACTION" <(awk '/^name:/{p=1} p' "$DEPLOYED") >/dev/null 2>&1; then
        ...
        echo "✗ deployed action drifts from source — run 1_workflows/build.sh"
        FAILS=$((FAILS + 1))
    fi
else
    echo "✗ deployed action missing — run 1_workflows/build.sh"
    FAILS=$((FAILS + 1))
fi
