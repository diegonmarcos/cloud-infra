...
# ║                                                                  ║
# ║   Source : 1_workflows/src/test/test_proxied_mcps_resolution.sh
...
# ║                                                                  ║
# ║ Usage: bash 1_workflows/src/test/test_proxied_mcps_resolution.sh ║
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
DERIVED_DIR="$REPO_ROOT/2_configs/dist"
...
for hub_json in "${HUBS[@]}"; do
  ...
  if [ ! -f "$derived" ]; then
    fail "$hub: derived $derived missing — run 2_configs/build.sh"
    continue
  fi
