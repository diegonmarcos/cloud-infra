    ...
    if [ "$HM_DELIVERY" = "docker" ] && [ "$HM_REMOTE_BUILDER" != "true" ]; then
        ...
        if [ "$FREE_GB" -lt "$MIN_FREE_GB" ]; then
            ...
            ssh_vm "sudo find /var/lib/docker/containers -name '*-json.log' -size +50M -exec truncate -s 0 {} + 2>/dev/null || true; \
                    ...
                    if ! systemctl cat containerd.service >/dev/null 2>&1 \
                       && [ -d /var/lib/containerd/io.containerd.content.v1.content ]; then \
                       echo '[pre-flight] orphan containerd detected (no service unit) — pruning content store'; \
