...
| **Local** `1_configs/build.sh consolidate` on my vault | ✅ emits `surface=hG1x8b4JXsD0r0TXVD…` (REAL) |
| CI gen-configs sees the vault | ✅ log: `vault/wireguard: 15 public keys` (same count as local) |
| CI gen-configs runs the full pipeline | ✅ log: `Using engine: 1_configs/build.sh all` → `Consolidating → /root/git/cloud/1_configs/dist/_cloud-data-consolidated.json` |
| **Committed** canonical `1_configs/dist/_cloud-data-consolidated.json` | ❌ `surface=NULL` (termux/.10/.11 = REAL) |
...
- `cloud-builder gen-configs` → `cloud-ship-orchestrate-gen-configs.sh` → `build.sh config`
  → `cloud-ship-repo-config-gen.sh` → **`bash $CLOUD_ROOT/1_configs/build.sh all`**.
...
So the derive writes the fresh `1_configs/dist/` (with `surface=REAL`) into the **container's
ephemeral** `/root/git/cloud/1_configs/dist`, which is **discarded on container exit**. The
workflow's next step —

```
git add 1_configs/dist/ b_infra/_shared/vm-pilot/dist/ b_infra/nixhm-sudo-*/dist/ \
...
- Use a writable **output mount** for just the dist trees, e.g.
  `-v "$GITHUB_WORKSPACE/1_configs/dist:/root/git/cloud/1_configs/dist"` (+ the
...
- Or, no mounts: after `docker compose run`, `docker cp <container>:/root/git/cloud/1_configs/dist/. 1_configs/dist/` (requires a non-`--rm` run + explicit name + cleanup).
...
deploy key and have the engine commit+push `1_configs/dist` + `b_infra/**/dist` from inside.
...
1. **Unit (local, already green):** `cd 1_configs && bash build.sh all` → assert
...
2. **Pipeline (the fix):** dispatch `ship-gen-configs`; assert it now produces a **commit**
   (not "nothing to commit") and that `git show origin/main:1_configs/dist/_cloud-data-consolidated.json`
...
- The `workflow_run` cascade path-gate (`ship-home-manager.yml` lines ~89-97) only fires HM on
  `b_infra/**` changes; a wg-public *client* add changes `1_configs/dist` (+ once Option A
...
`1_configs/dist/_cloud-data-consolidated.json` now carries
...
```
1_configs/dist/_cloud-data-consolidated.json            (canonical — now has surface)
...
2. The committed per-VM `dist/pilot` snapshot should not be a stale regular file at all — it should
   ...
   regenerated+committed by gen-configs (it currently isn't — `1_configs/build.sh all` writes the
...
a symlink to `../../../1_configs/dist/_cloud-data-consolidated.json` (always current, matches the
