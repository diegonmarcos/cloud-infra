...
| R7 | VM sanity: `grep -l ^_ /opt/containers/*/.secrets` returns empty | ⏳ not yet run |
...
| # | task | priority |
|---|------|----------|
| L1 | Root-cause the silent `git reset --hard` that wiped the working tree twice (likely `hooks/cloud-git-sync.sh`); audit its triggers | medium |
| L2 | Document in `~/.claude/CLAUDE.md` that long editing sessions must start with `git status --porcelain` + a manual tag of HEAD | low |
| L3 | Verify `lint-secrets-coverage.yml` GHA workflow runs green on the next PR | low |
| L4 | VM sanity: on each VM, `grep -l ^_ /opt/containers/*/.secrets` → must be empty (proves `_credentials` engine filter works end-to-end) | medium |
| L5 | Clone-scan for other `*.yaml.new` / `*.bak` plaintext leftovers | low |
