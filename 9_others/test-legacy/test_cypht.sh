...
# Run after `cloud/9_others/build.sh` regenerates dist/ and after
...
if [ -f "$CADDY_JSON" ]; then
    ...
    fail_with "build-caddy.json not found — run cloud/9_others/build.sh first"
fi
...
BC_JSON="$CLOUD_ROOT/1_cicd/dist/build-cypht.json"
