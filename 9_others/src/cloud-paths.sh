...
cloud_paths_gha_config() {
    ...
    _cloud_paths_first_existing "build-gha.json (or legacy cloud-data-gha-config.json)" \
        ...
        "$_cpgc_root/1_cicd/dist/cloud-data-gha-config.json" \
...
# 9_others/build.json (.runners + .probe + .dispatch), aggregated by
# 9_others/build.sh into dist/build-workflows.json. Schema unchanged:
...
cloud_paths_runners() {
    ...
    _cloud_paths_first_existing "build-workflows.json" \
        ...
        "$_cpr_root/9_others/src/build-workflows.json"
}
