...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_build_schema_api_mcp.sh
...
# ║                                                                  ║
# ║ Usage: bash 9_others/test/test_build_schema_api_mcp.sh    ║
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail
...
VALIDATOR="$REPO_ROOT/1_cloud-configs/src/derive/validate-build-schema.ts"
...
HARNESS="$REPO_ROOT/1_cloud-configs/src/derive/_validate-block-fixture.ts"
...
run_harness() {  # block, fixture-content -> echoes VALID|INVALID (always exit 0)
  ...
  (cd "$REPO_ROOT/1_cloud-configs/src/derive" && npx tsx "$HARNESS" "$SCHEMA" "$fpath" "$block" 2>&1) || true
}
...
if (cd "$REPO_ROOT/1_cloud-configs/src/derive" && npx tsx "$VALIDATOR" >/dev/null 2>&1); then
