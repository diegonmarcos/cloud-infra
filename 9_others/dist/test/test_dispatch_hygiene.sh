...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_dispatch_hygiene.sh
...
# ║   I) only dispatch updates 1_cicd/dist submodule; nix step      ║
# ║      honours CLOUD_DATA_PRESTAGED_BY_CI flag                     ║
# ║                                                                  ║
# ║ Usage: bash 9_others/test/test_dispatch_hygiene.sh        ║
...
if grep -q 'submodule update --remote --init --force 1_cicd/dist' "$SCRIPTS/cloud-ship-ci-builder-dispatch.sh"; then
