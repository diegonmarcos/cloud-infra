...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_no_dead_nix_install_branch.sh
...
# ║                                                                  ║
# ║ Usage: bash 9_others/test/test_no_dead_nix_install_branch.sh
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail
...
LIB="$REPO_ROOT/1_cicd/src/scripts/cloud-ship-lib.sh"
...
while IFS= read -r f; do
    ...
    while IFS= read -r line; do
        HITS+=("$rel:$line")
             ...
             | grep -vE '^[0-9]+:\s*//' \
             || true)
done < <(find "$REPO_ROOT/1_cicd/src/scripts" -name 'cloud-ship-*.sh' -type f 2>/dev/null)
