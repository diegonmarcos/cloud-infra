...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_engine_steps_tracked.sh
...
# ║                                                                   ║
# ║ Usage: bash 9_others/test/test_engine_steps_tracked.sh     ║
# ╚══════════════════════════════════════════════════════════════════╝
set -eo pipefail
...
# Excluded:
#   9_others/test/    (tests mention filenames as fixtures, e.g. heredocs)
#   1_cicd/dist/        (generated mirror of src/ — duplicate references)
# Included:
#   per-service build.sh symlinks → consolidated engine
#   1_cicd/src/scripts/ engines/orchestrators
#   1_cicd/src/cicd/    workflow source
#   .github/workflows/       generated workflows (also tracked)
mapfile -t scan_files < <(
  git ls-files \
    '1_cicd/src/scripts/*.sh' \
    '1_cicd/src/cicd/*.yml' \
...
for step in "${referenced[@]}"; do
  ...
  # from 1_cicd/src/scripts/. dist/scripts is the generated mirror used
  ...
  src_path="1_cicd/src/scripts/$step"
  dist_path="1_cicd/dist/scripts/$step"
