...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_fetch_vault_action.sh
...
if [ -f "$DEPLOYED" ]; then
    ...
    if diff -q "$ACTION" <(awk '/^name:/{p=1} p' "$DEPLOYED") >/dev/null 2>&1; then
        ...
        echo "✗ deployed action drifts from source — run 9_others/build.sh"
        FAILS=$((FAILS + 1))
    fi
else
    echo "✗ deployed action missing — run 9_others/build.sh"
    FAILS=$((FAILS + 1))
fi
