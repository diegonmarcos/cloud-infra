...
# ║                                                                  ║
# ║   Source : 9_others/src/../test/test_repo_config_gen_engine_resolution.sh
...
#   <p>/1_cicd/src/scripts/cloud-data-config.ts
# inside three legacy roots (1_cicd/dist, sibling cloud-data,
...
# became cloud/9_others/build.sh. Every gen-configs CI run logged
...
#   1. The script tries cloud/9_others/build.sh FIRST (current layout).
...
REPO="$_d"
SCRIPT="$REPO/1_cicd/src/scripts/cloud-ship-repo-config-gen.sh"
FAILS=0

[ -f "$SCRIPT" ] || { echo "✗ $SCRIPT not found"; exit 1; }

# 1. Current layout — 9_others/build.sh dispatch is present and FIRST.
if grep -q '9_others/build.sh.*all\|"$CLOUD_ROOT/9_others/build.sh"' "$SCRIPT"; then
    echo "✓ script dispatches to 9_others/build.sh (current engine layout)"
else
    echo "✗ script does not dispatch to 9_others/build.sh — engine reorg not handled"
    FAILS=$((FAILS + 1))
fi

# Path A must come before Path B in the script (current preferred over legacy).
path_a_line=$(grep -n '9_others/build.sh' "$SCRIPT" | head -1 | cut -d: -f1)
...
if grep -q 'cloud-data-config\.ts' "$SCRIPT"; then
    echo "✓ legacy fallback (1_cicd/src/scripts/cloud-data-config.ts) still present"
else
    echo "✗ legacy fallback removed — back-compat broken"
    FAILS=$((FAILS + 1))
fi

# 3. Resolves on the actual checkout: 9_others/build.sh exists and is executable
if [ -x "$REPO/9_others/build.sh" ]; then
    echo "✓ 9_others/build.sh is executable on this checkout"
else
    echo "✗ 9_others/build.sh missing or non-executable — engine reorg incomplete"
    FAILS=$((FAILS + 1))
fi
...
if grep -q 'SKIP: neither 9_others/build.sh nor cloud-data legacy engine found' "$SCRIPT"; then
