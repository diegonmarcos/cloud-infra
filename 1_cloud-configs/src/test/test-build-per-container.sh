...
CLOUD_DATA_DIR="${CLOUD_DATA_DIR:-$REPO/cloud/1_cloud-configs/dist}"
...
for BF in "${BUILD_FILES[@]}"; do
  ...
  mapfile -t CONSUMERS < <(command find "$CLOUD_SOLUTIONS_DIR" -maxdepth 3 -lname "*1_cloud-configs/dist/$BF" -o -lname "*I_cloud-data/$BF" 2>/dev/null)
