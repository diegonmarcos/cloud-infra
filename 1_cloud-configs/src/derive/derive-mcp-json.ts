...
function main(): void {
  ...
  const out = {
    ...
    _doc: 'Project-scoped MCP servers for Claude Code. In cloud, 1_configs/build.sh refreshes src/apps/root/mcp.json from THIS file and deploys it to .mcp.json, so regenerating the derive is enough. In the other repos the copy under 1_configs/src/apps/root/ is propagated by hand — nothing automates that hop, and it is how vault ended up without the c3-infra-mcp headersHelper while cloud had it. Re-sync after changing this file.',
    _skipped: skipped,
